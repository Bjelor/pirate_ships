...
class PirateShipsRepository(
    ...
    private val pirateShipFactory: PirateShipFactory by lazy { PirateShipFactory() }

    private val pirateShipEntityFactory: PirateShipEntityFactory by lazy {
    ...
    fun fetchPirateShips(): Flow<List<PirateShip>> = flow {
        val shipEntities =
            pirateShipDao.getAll().takeIf { it.isNotEmpty() } ?: downloadPirateShips()
